...
public class ByteArrayUtils {
    ...
    public static final int   SIZE_OF_CHAR_IN_BYTES  = 2;

    public static void writeChar(byte[] p_dest, char p_toWrite) 
    {
	assert( p_dest.length >= SIZE_OF_AN_INT_IN_BYTES )
	    : "Programming error: p_dest is too short to hold a char";
    ...
    public static char readChar(byte[] p_src) // must be of size 2 
    {
	assert( p_src.length >= SIZE_OF_CHAR_IN_BYTES )
	    : "Programming error: p_src is too short to hold a char";
	
	// unrolled loop of 2 iterations
	char result = (char)(   p_src[ 0 ] &  MASK_TO_BYTE);
	result    |= (char)( ( p_src[ 1 ] & MASK_TO_BYTE ) << 8 );
